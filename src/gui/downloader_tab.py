...
from ..yt_dlp_worker import PlaylistFirstVideoAnalyzer, VideoAnalyzer
...
class DownloaderTab(QWidget):
    def __init__(self, main_window: QWidget):
        ...
        self.first_video_analyzer: PlaylistFirstVideoAnalyzer | None = None
    ...
    def _init_ui(self):
        ...
        self.noplaylist_check = QCheckBox(
            "Ignore playlist context and download only the single video"
        )
        ...
        self.audio_quality_combo.addItems(
            ["320 kbps (High)", "256 kbps", "192 kbps (Medium)", "128 kbps (Low)"]
        )
    ...
    def _on_speed_test_finished(self, speed: float, speed_str: str):
        ...
        QMessageBox.information(
            self, "Speed Test", f"Download speed test completed!\n\nMeasured Speed: {speed_str}"
        )
        self.speed_tester = None
    ...
    def _on_speed_test_error(self, error_msg: str):
        ...
        self.speed_test_btn.setText("Speed Test")
        QMessageBox.warning(
            self, "Speed Test Failed", f"Could not complete speed test:\n{error_msg}"
        )
        self.speed_tester = None
    ...
    def _start_analysis(self):
        ...
        if self.analyzer:
            ...
            self.analyzer.finished.connect(
                lambda a=self.analyzer: self._handle_analyzer_terminated(a)
            )
            self.analyzer = None
        ...
        for f in formats:
            ...
            if is_video:
                ...
                self.audio_formats.append(
                    {"id": f_id, "ext": ext, "abr": abr, "desc": f"{abr_desc} ({ext})"}
                )
    ...
    def _build_ydl_opts(self) -> dict[str, Any]:
        ...
        if config_manager.get("bypass_throttling", True):
            opts["extractor_args"] = {
                "youtube": {"player_client": ["web", "mweb", "android", "ios"]}
            }
    ...
    def _save_to_library(self):
        ...
        duration = (
            self._format_duration(self.current_analysis.get("duration", 0))
            if self.current_analysis.get("duration")
            else "Unknown"
        )
        ...
        if success:
            QMessageBox.information(
                self, "Saved to Library", f"Successfully saved '{title}' to your library!"
            )
            if hasattr(self.main_window, "library_tab"):
                self.main_window.library_tab.refresh_library()
        else:
            QMessageBox.warning(
                self, "Already Saved", "This video/playlist is already saved in your library."
            )
    ...
    def _on_task_updated_downloader(self, task_id: str, data: dict):
        ...
        if "downloaded" in data or "total" in data:
            ...
            self.progress_size_label.setText(
                f"{task.get('downloaded', '0 B')} / {task.get('total', 'Unknown')}"
            )
        ...
        if "status" in data:
            ...
            if status == "Downloading":
                ...
                self.progress_bar.setStyleSheet(
                    "QProgressBar::chunk { background-color: #71717a; }"
                )  # muted gray
                self.progress_pause_btn.setVisible(False)
                self.progress_cancel_btn.setVisible(False)
                self.progress_done_btn.setVisible(True)
    ...
    def _get_playlist_entries(self) -> list:
        ...
        entries = self.current_analysis.get("_entries_list") or self.current_analysis.get(
            "entries", []
        )
        if entries is None:
            return []
    ...
    def _is_playlist(self) -> bool:
        ...
        if not self.current_analysis:
            return False
        return (
            self.current_analysis.get("_type") == "playlist" or "entries" in self.current_analysis
        )
    ...
    def _load_first_video_formats(self):
        ...
        if self.first_video_analyzer and self.first_video_analyzer.isRunning():
            ...
            self.first_video_analyzer.finished.connect(
                lambda a=self.first_video_analyzer: self._handle_analyzer_terminated(a)
            )
    ...
    def _start_download_now_task(
        self, url: str, title: str, thumb_path: str | None, ydl_opts: dict[str, Any]
    ):
