...
class YtdlLogger:
    ...
    def debug(self, msg: str):
        if msg.startswith("[download]") and "%" in msg:
            return
        if hasattr(self.worker, "log_received"):
        ...
        try:
            ...
            if self._cancelled:
                ...
                self.download_finished.emit(
                    self.current_filename or "Download completed successfully."
                )
...
class PlaylistFirstVideoAnalyzer(QThread):
    ...
    def __init__(self, video_url: str):
        super().__init__()
        self.video_url = sanitize_url(video_url)
        self.cancelled = False
...
class PlaylistFormatPreCheckWorker(QThread):
    ...
    check_completed = Signal(list, list)
    ...
    def __init__(
        ...
        super().__init__()
        self.playlist_url = sanitize_url(playlist_url)
